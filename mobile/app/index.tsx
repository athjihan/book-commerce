      ...
      <ScrollView >
        <View className="flex flex-wrap flex-row justify-between">
          {paginatedBooks.map((book, index) => (
            ...
            >
              <Image
                source={require("@/assets/cover/umugm.png")}
              ...
              {book.book_type === "e-book" ? (
                <View className="bg-green-300 px-2 py-0.5 rounded-full mt-1 self-start">
                  <Text className="text-xs italic text-green-900">{book.book_type}</Text>
                </View>
              ) : book.book_type === "physics" ? (
                <View className="bg-red-200 px-2 py-0.5 rounded-full mt-1 self-start">
                  <Text className="text-xs italic text-red-800">{book.book_type}</Text>
                </View>
              ) : (
